console.log("Google Meet content script loaded!");
...
// Function to detect Google Meet participants
function detectMeetParticipants() {
    console.log("detectMeetParticipants called. Extension enabled:", isExtensionEnabled);
    ...
    const participantNameElements = document.querySelectorAll('.ZjFb7c'); // Selector for Google Meet names
    console.log("Found participant name elements in Meet:", participantNameElements.length);

    participantNameElements.forEach(element => {
        const participantName = element.textContent.trim();
        if (participantName) {
            console.log("Detected Meet participant:", participantName);
            currentParticipants.add(participantName);

            if (!nameTagPopups.has(participantName)) {
                console.log("Creating name tag for Meet participant:", participantName);
    ...
    nameTagPopups.forEach((popup, name) => {
        if (!currentParticipants.has(name)) {
            console.log("Removing name tag for Meet participant:", name);
            popup.remove();
...
const debouncedDetectMeetParticipants = debounce(detectMeetParticipants, 3000);

// MutationObserver to watch for changes in the DOM
const observer = new MutationObserver((mutations) => {
    mutations.forEach((mutation) => {
        if (mutation.addedNodes.length || mutation.removedNodes.length) {
            debouncedDetectMeetParticipants();
        }
    });
});
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "stateChanged") {
        ...
        if (!isExtensionEnabled) {
            ...
            debouncedDetectMeetParticipants(); // Re-detect if enabled
        }
    }
});
